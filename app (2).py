...
# Optional: client-side persistence (no accounts) for Streamlit Community Cloud
try:
...
def load_profile() -> Dict[str, Any]:
    if os.path.exists(PROFILE_PATH):
        with open(PROFILE_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"nombre": "", "rut": "", "especialidad": "", "icm": "", "institucion": "", "correo": "", "telefono": "", "stamp_font_size": 9}
        ...
        for field_key, bind in bindings.items():
            ...
            if btype in ("lr_singleline", "lr_multiline"):
                ...
                if isinstance(raw, bool):
                    ...
                    checked = s in ("1", "true", "t", "si", "sí", "x", "check", "checked", "on")
...
def validate_ui_rules(tpl: Dict[str, Any], caso: Dict[str, Any]) -> List[str]:
    """Validate optional tpl['ui']['rules'].

    Supported:
      - {type:'xor', keys:[a,b,...], message:'...'} : exactly one True
      - {type:'at_least_one', keys:[...], message:'...'} : at least one True
    """
    ui = tpl.get("ui") or {}
    rules = ui.get("rules") or []
    ...
    for r in rules:
        ...
        if rtype == "xor":
            if truthy != 1:
                errs.append(msg or f"Regla XOR falló: {', '.join(keys)}")
        elif rtype == "at_least_one":
            if truthy < 1:
                errs.append(msg or f"Debe marcar al menos una opción: {', '.join(keys)}")
        # ignore unknown rule types (forward-compatible)
    return errs
...
def render_dynamic_case_form(tpl: Dict[str, Any], perfil: Dict[str, Any]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    # Prefill: by default, "tratante 1" is the same doctor filling the form (perfil)
    prefill: Dict[str, Any] = {}
    try:
        nombre_med = (perfil.get("nombre") or "").strip()
        if nombre_med:
            prefill["trt1_nombre"] = f"Dr. {nombre_med}"
        else:
            prefill["trt1_nombre"] = ""
        prefill["trt1_prof"] = (perfil.get("especialidad") or "").strip()
        prefill["trt1_rut"] = (perfil.get("rut") or "").strip()
        prefill["trt1_tel"] = (perfil.get("telefono") or "").strip()
    except Exception:
        prefill = {}
        ...
        with st.expander(sec_name, expanded=True):
            for f in sec_fields:
                ...
                if ftype == "text":
                    ...
                    dflt = default if isinstance(default, date) else date.today()
                    caso[key] = st.date_input(label, value=dflt, help=help_txt, key=f"case_{key}")
                elif ftype == "number":
                    minv = f.get("min", None)
                    maxv = f.get("max", None)
                    step = f.get("step", 1)
                    # Streamlit needs numeric defaults; if missing, pick min or 0
                    if default is None:
                        if minv is not None:
                            default_num = float(minv)
                        else:
                            default_num = 0.0
                    else:
                        try:
                            ...
                            default_num = 0.0
                    caso[key] = st.number_input(label, value=default_num,
                    ...
                    # Streamlit checkbox expects a bool
...
st.title("MVP Formularios — L/R (Opción B) + Campos dinámicos (sin romper lo existente)")
...
with tabs[0]:
    ...
    # Optional: remember profile in this browser (no accounts) — useful on Streamlit Community Cloud
    local_storage = LocalStorage() if LocalStorage else None
    if local_storage and not st.session_state.get("_ls_profile_loaded", False):
        try:
            raw = local_storage.getItem("mvp_profile")
            if raw:
                parsed = json.loads(raw)
                if isinstance(parsed, dict):
                    # Merge, keeping file-based values as fallback
                    for k, v in parsed.items():
                        if k not in perfil or perfil.get(k, "") in ("", None):
                            perfil[k] = v
            st.session_state["_ls_profile_loaded"] = True
        except Exception:
            st.session_state["_ls_profile_loaded"] = True
    ...
    remember = st.checkbox("Recordar este perfil en este navegador", value=bool(st.session_state.get("_remember_profile", True)))
    st.session_state["_remember_profile"] = remember

    if local_storage and not remember:
        if st.button("Olvidar perfil guardado en este navegador"):
            try:
                local_storage.deleteItem("mvp_profile")
            except Exception:
                pass
            st.success("Perfil olvidado en este navegador.")

    if st.button("Guardar perfil"):
        save_profile(perfil)
        # Persist to browser localStorage if available and enabled
        if local_storage and remember:
            try:
                local_storage.setItem("mvp_profile", json.dumps(perfil, ensure_ascii=False))
            except Exception:
                pass
...
with tabs[1]:
    if not installed:
        ...
        if ui_fields:
            ...
            rule_errors = validate_ui_rules(tpl, caso)
        else:
            missing_labels = []
            rule_errors = []
        ...
        with colB:
            if st.button("Generar PDF final"):
                ...
                if err:
                    ...
                    if ui_fields and (missing_labels or rule_errors):
                        if missing_labels:
                            st.error("Faltan campos requeridos: " + ", ".join(missing_labels))
                        if rule_errors:
                            st.error("Reglas del formulario: " + " | ".join(rule_errors))
                    else:
